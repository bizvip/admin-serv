...
class AutoFromMapper
{
     ...
     * 过滤查询字段不存在的属性.
     */
    public function filterQueryAttributes(
        string $pk,
        array $columns,
        array $fields,
        bool $removePk = false,
     ...
     * 过滤新增或写入不存在的字段.
     */
    public function filterExecuteAttributes(
        string $pk,
        array $columns,
        array &$data,
        bool $removePk = false,
    ...
    public function disable(mixed $table_id, array $ids, string $field = 'status'): bool
    {
        ...
        Db::table($table->getTableName())
            ->whereIn($pkColumn->column_name, $ids)
            ->update([$field => 2]);
    ...
    protected function userDataScope($query, ?int $userid = null, $dataScopeField = 'created_by')
    {
        ...
        $dataScope = new class($userid, $query, $dataScopeField) {
            ...
            public function execute(): Builder
            {
                ...
                return empty($this->userIds) ? $this->builder : $this->builder->whereIn($this->dataScopeField, array_unique($this->userIds));
            }
                    ...
                    switch ($role->data_scope) {
                        ...
                        case SystemRole::DEPT_BELOW_SCOPE:
                            ...
                            foreach ($parentDepts as $deptId) {
                                $ids[] = SystemDept::query()->where(function ($query) use ($deptId,
                                    ) {
