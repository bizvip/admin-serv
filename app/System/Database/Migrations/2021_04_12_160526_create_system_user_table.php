...
class CreateSystemUserTable extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('system_user', function (Blueprint $table) {
            ...
            $table->addColumn('string', 'nickname', ['length' => 30, 'comment' => '用户昵称'])
                ->nullable();
            $table->addColumn('string', 'phone', ['length' => 11, 'comment' => '手机'])->nullable();
            $table->addColumn('string', 'email', ['length' => 50, 'comment' => '用户邮箱'])
                ->nullable();
            $table->addColumn('string', 'avatar', ['length' => 255, 'comment' => '用户头像'])
                ->nullable();
            $table->addColumn('string', 'signed', ['length' => 255, 'comment' => '个人签名'])
                ->nullable();
            $table->addColumn('string', 'dashboard', ['length' => 100, 'comment' => '后台首页类型'])
                ->nullable();
