    ...
    public function up(): void
    {
        if (! Schema::hasTable('setting_datasource')) {
            Schema::create('setting_datasource', function (Blueprint $table) {
                ...
                $table->addColumn('string', 'source_name', [
                    'length' => 32,
                    'comment' => '数据源名称',
                ]);
                $table->addColumn('string', 'dsn', ['length' => 255, 'comment' => '连接dsn字符串'])
                    ->nullable();
                $table->addColumn('string', 'username', ['length' => 64, 'comment' => '数据库名称'])
                    ->nullable();
                $table->addColumn('string', 'password', ['length' => 32, 'comment' => '数据库用户'])
                    ->nullable();
